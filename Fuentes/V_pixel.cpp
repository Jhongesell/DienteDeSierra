...
void Arreglo_pixeles::Asigna_linea(const int xi, const int yi, const int xf, const int yf, const bool val)
{
     ...
     // Linea vertical
     if (xxi == xxf) {
        for (i = min_y; i <= max_y; i++) Asigna_valor(xi, i, val);
     }
     // Linea horizontal
     if (xyi == xyf) {
        for (i = min_x; i <= max_x; i++) Asigna_valor(i, yi, val);
     }
     // Linea en general
     if (xxi != xxf && xyi != xyf) {
...
// Asigna linea
void Arreglo_pixeles::Asigna_linea(const long double xi, const long double yi, const long double xf, const long double yf, const bool val)
{
   int p1, p2, p3, p4;
///////////////////////////////////////////////////////////////////////////////
